...
import { RequestListSpecChangeForm } from "./requestListSpecChangeForm";
...
export const dispatchAction = action(function dispatchAction(action, helpers) {
    ...
    case 'requestListSpecChange':
      {
        ...
        helpers.requestPopupDialog('', <RequestListSpecChangeForm state={state} />).then((ok) => {
          if (!ok) return
      ...
      break
    case 'changeListSpec':
      updateList(action.list, { surgeonId: action.newSurgeon, specialtyId: action.newSpecialty })
      break
    case 'requestNewList':
      {
        const state = observable({
