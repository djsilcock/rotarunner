...
const DutyCell = observer(function DutyCell({
  ...
  return (
    <td className={classNames.dutyCell}>
      <span className={isSameMonth(startTime,viewDate)?classNames.dateNumber:classNames.otherMonthDateNumber}>{startTime.getDate()}</span>
        {duties.map(duty=><Duty key={duty.id} {...duty}/>)}
    </td>
  );
});
function MonthView({viewDate,getDuties}){
    const weeksInMonth=eachWeekOfInterval({start:startOfMonth(viewDate),end:endOfMonth(viewDate)},{weekStartsOn:1})
