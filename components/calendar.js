...
export default function Calendar({value,month,year}){
    ...
    return <div className={classNames.calendar} >
        <div>S</div><div>M</div><div>Tu</div><div>W</div><div>Th</div><div>F</div><div>Sa</div>
        {Array.from((function*(){
            for (let day=daysInPreviousMonth-firstDayOfMonthWeekday+1;day<=daysInPreviousMonth;day++){
